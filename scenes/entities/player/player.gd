...
var desert = load("res://scenes/levels/inherited levels/desert/overworldDesert.tscn")
...
var health = 5:
	set(value):
		...
		if health <= 0:
			...
			await get_tree().create_timer(0.5).timeout  # Small delay before restart
...
func next_level():
	if Input.is_action_just_pressed("nextscene"):
		get_tree().change_scene_to_packed(desert)
