...
@onready var attack_label: Label = $Bonus/AttackLabel
@onready var hp_label: Label = $Bonus/HPLabel
@onready var magic_label: Label = $Bonus/MagicLabel


func _ready() -> void:
	update_display()
	Globallvl.level_changed.connect(update_display)
	Globallvl.exp_changed.connect(update_display)
	
func update_display():
	level_label.text = "Level: %d" % Globallvl.current_level
	xp_bar.value = Globallvl.current_exp
	xp_bar.max_value = Globallvl.exp_to_next_level
